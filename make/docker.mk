...
build: ## Build the docker image with already downloaded image in docker cache
	make check-missing-env
	env $(cat .env | grep -v '^#') docker compose build php --pull --build-arg CURRENT_UID=$(CURRENT_UID)

.PHONY: build-no-cache
build-no-cache: ## Rebuild the docker image without docker cached images
	make check-missing-env
	env $(cat .env | grep -v '^#') docker compose build php --pull --no-cache --build-arg CURRENT_UID=$(CURRENT_UID)
